...
from schemas import UserCreate, Userupdate,PostResponse, UserPublic, UserPrivate, Token, PaginatedPostsResponse, ChangePasswordRequest, ForgotPasswordRequest, ResetPasswordRequest

from datetime import timedelta , UTC, datetime
from fastapi.security import OAuth2PasswordRequestForm

from auth import hash_password, verify_password, create_token_access, CurrentUser, generate_reset_token, hash_reset_token #, verify_access_token, oauth2_scemema
...
async def create_user(user: UserCreate, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    if existing_user:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="UserName Already exist please try with other UserName")
    
    result = await db.execute(select(models.User).where(func.lower(models.User.email) == user.email.lower()))
    existing_email = result.scalars().first()

    if existing_email:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Email Already exist please try with other Email")
...
async def login_for_access_token(form_data: Annotated[OAuth2PasswordRequestForm, Depends()], db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    # Create access Token with User_ID as sub
...
# before implement current user
# async def get_current_user( token: Annotated[str, Depends(oauth2_scemema)], db: Annotated[AsyncSession, Depends(get_db)] ):
# After implement current user from token
...
async def get_user_ByID(user_id:int, db: Annotated[AsyncSession, Depends(get_db)]):
...
async def get_user_posts(user_id:int, db: Annotated[AsyncSession, Depends(get_db)], skip: Annotated[int, Query(ge=0)] = 0, limit: Annotated[int, Query(ge=1, le=100)] = 5,):
...
async def upload_profile_picture(user_id: int, file: UploadFile, current_user : CurrentUser, db : Annotated[AsyncSession, Depends(get_db)]):
    ...
    if current_user.id != user_id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not Authorized to update this user's pitcture ")
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid image File. Please upload valid Image (JPEG, PNG, GIF, webP).") from err
...
async def delete_user_profile(user_id: int, current_user : CurrentUser, db : Annotated[AsyncSession, Depends(get_db)]):
    ...
    if old_filename is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No profile pitcure to delete.")
