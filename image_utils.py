...
def process_profile_image(content: bytes) -> tuple[bytes, str]:    #-> str:

    with Image.open(BytesIO(content)) as original:

        # img = ImageOps.fit(img, (300, 300), method=Image.Resampling.LANCZOS)
        img = ImageOps.fit(original,(300, 300),method=Image.Resampling.LANCZOS)
